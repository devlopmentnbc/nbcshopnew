...
@section('title', 'Edit Promotion - Admin NBC')

@section('content')
    <main class="min-h-[calc(100vh-140px)] px-4 py-6 lg:px-6">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-[24px] font-semibold text-ink-900">Edit Promotion</h1>
                <p class="mt-1 text-[14px] text-ink-500">Update {{ $promotion->name }} and its active schedule.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <div class="space-y-6 lg:col-span-2">
                    <section class="space-y-5">
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text" required maxlength="255"
                                value="{{ old('name', $promotion->name) }}" placeholder="e.g. Avocado Body Care Weekend"
                        ...
                        <!-- Small Promotion Label and Button Text -->
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <div>
                                <label for="promotion_label" class="mb-2 block text-[14px] font-semibold text-ink-900">Small
                                    Promotion Label
                                    <span class="text-danger-500">*</span></label>
                                <input id="promotion_label" name="promotion_label" type="text" required maxlength="255"
                                    value="{{ old('promotion_label', $promotion->promotion_label) }}"
                            ...
                            /
                            <div>
                                <label for="button_text" class="mb-2 block text-[14px] font-semibold text-ink-900">Button
                                    Text
                                    <span class="text-danger-500">*</span></label>
                        ...
                        <div>
                            ...
                            <input id="slug" name="slug" type="text" maxlength="255"
                                value="{{ old('slug', $promotion->slug) }}" placeholder="avocado-body-care-weekend"
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="6" maxlength="5000"
                                ...
                                class="w-full rounded-base border border-surface-line bg-surface-body p-4 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none @error('description') border-danger-500 @enderror">{{ old('description', $promotion->description) }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-[13px] text-danger-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <section class="space-y-5 border-t border-surface-line pt-6">
                        ...
                        <div>
                            ...
                            <select id="target_type" name="target_type" required
                                ...
                                @foreach ($targetTypes as $value => $label)
                                    <option value="{{ $value }}" @selected(old('target_type', $promotion->target_type) === $value)>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            @error('target_type')
                        ...
                        <div id="promotion-target-wrapper">
                            ...
                            <input id="target_id" name="target_id" type="hidden"
                                value="{{ old('target_id', $promotion->target_id) }}">
                ...
                <! -- Schedule and Image -->
                    <div class="space-y-6 border-t border-surface-line pt-6 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
                        <section class="space-y-5">
                            <h2 class="border-b border-surface-line pb-3 text-[18px] font-semibold text-ink-900">Schedule
                            </h2>

                            <div>
                                <label for="starts_at" class="mb-2 block text-[14px] font-semibold text-ink-900">Starting
                                    Date &
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" data-open-picker="starts_at"
                                        aria-label="Open starting date calendar"
                            ...
                            <div>
                                <label for="ends_at" class="mb-2 block text-[14px] font-semibold text-ink-900">Ending
                                    Date &
                                    Time <span class="text-danger-500">*</span></label>
                                <div class="relative">
                                    ...
                                    <button type="button" data-open-picker="ends_at"
                                        aria-label="Open ending date calendar"
                                        class="absolute right-1 top-1/2 inline-flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-base text-brand-600 hover:bg-brand-50">
                                        <i data-lucide="calendar-clock" class="h-5 w-5"></i>
                                    </button>
                                </div>
                            ...
                            <label
                                ...
                                <input type="checkbox" name="status" value="1" @checked(old('status', $promotion->status))
                                ...
                                <span>
                                    ...
                                    <small class="text-[12px] text-ink-500">Scheduling only takes effect while
                                        enabled.</small>
                                </span>
                            </label>
                        </section>
                        ...
                        <section class="space-y-4 border-t border-surface-line pt-6">
                            ...
                            <label for="image"
                                ...
                                <img id="image-preview" src="{{ asset($promotion->image) }}" alt="Promotion preview"
                                    class=" h-full w-full rounded-base object-contain">
                                <span id="image-upload-placeholder" class="hidden flex-col items-center">
                                    <span
                            ...
                            <p id="image-file-name" class="truncate text-[12px] font-medium text-brand-600">
                                {{ basename($promotion->image) }}</p>
                            @error('image')
                                <p class="text-[13px] text-danger-500">{{ $message }}</p>
                            @enderror
                            <p class="text-[12px] text-ink-400">JPEG, PNG, WEBP or GIF. Maximum 5MB. Wide banner images
                                work
                                best.</p>
                        </section>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const selectedTarget = @json((string) old('target_id', $promotion->target_id));
