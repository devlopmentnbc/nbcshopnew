                ...
                <div class="rounded-card border border-surface-line bg-surface-card p-6 shadow-card space-y-6">
                    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-surface-line pb-3">
                        ...
                        <div class="flex items-center gap-2 text-[12px]">
                            <button type="button" onclick="expandAllAttrGroups('editAttributesList')" class="font-medium text-brand-600 hover:underline">Expand All</button>
                            <span class="text-ink-300">|</span>
                            <button type="button" onclick="minimizeAllAttrGroups('editAttributesList')" class="font-medium text-ink-500 hover:underline">Minimize All</button>
                        </div>
                    </div>

                    @if ($attributes->count() > 0)
                        <div id="editAttributesList" class="space-y-4">
                                    ...
                                    <div class="attr-group-content p-4 border-t border-surface-line space-y-3">
                                        @forelse ($attr->values as $val)
                                            ...
                                            <div class="variant-item rounded-base border border-surface-line bg-surface-card p-4">
                                                ...
                                                <div id="variant_inputs_{{ $val->id }}" class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-3 pt-2 border-t border-surface-line {{ old("variants.{$val->id}.selected", $isAttached) ? '' : 'hidden' }}">
                                                    <!-- LKR Price -->
                                                    <div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="number" step="1" min="0" name="variants[{{ $val->id }}][weight_grams]" value="{{ old("variants.{$val->id}.weight_grams", $varData['weight_grams'] ?? '') }}" placeholder="e.g. 500" class="h-9 w-full rounded-base border border-surface-line bg-surface-body px-3 text-[13px] text-ink-900 focus:border-brand-600 focus:outline-none">
                                                    </div>
