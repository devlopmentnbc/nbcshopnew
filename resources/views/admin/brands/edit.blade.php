...
@section('title', 'Edit Brand - Admin Unimart')
...
<main class="px-4 py-6 lg:px-6 min-h-[calc(100vh-140px)]">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-[24px] font-semibold text-ink-900">Edit Brand</h1>
            <p class="mt-1 text-[14px] text-ink-500">Update brand details and logo image.</p>
        </div>
        <div>
    ...
    <div class="max-w-2xl rounded-card border border-surface-line bg-surface-card p-6 shadow-card">
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $brand->name) }}" required placeholder="e.g. Nike, Apple, Sony" class="h-11 w-full rounded-base border border-surface-line bg-surface-body px-4 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none @error('name') border-danger-500 @enderror">
                @error('name')
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $brand->slug) }}" placeholder="e.g. nike, apple" class="h-11 w-full rounded-base border border-surface-line bg-surface-body px-4 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none @error('slug') border-danger-500 @enderror">
                @error('slug')
                    <p class="mt-1.5 text-[13px] text-danger-500">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="flex items-center gap-4 mb-3">
                    @if ($brand->image)
                        <div class="h-20 w-20 shrink-0 overflow-hidden rounded-base border border-surface-line bg-surface-body p-1">
                            <img id="current-image" src="{{ asset($brand->image) }}" alt="{{ $brand->name }}" class="h-full w-full object-contain">
                        </div>
                    @endif
                    <div id="image-preview-container" class="hidden h-20 w-20 shrink-0 overflow-hidden rounded-base border border-brand-500 bg-surface-body p-1">
                        <img id="image-preview" src="#" alt="New Preview" class="h-full w-full object-contain">
                    </div>
                </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <input type="checkbox" name="status" id="status" value="1" {{ old('status', $brand->status) ? 'checked' : '' }} class="h-4 w-4 rounded border-surface-line text-brand-600 focus:ring-brand-600">
