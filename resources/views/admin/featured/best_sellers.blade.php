...
@section('title', 'Best Sellers Products - Admin Unimart')
...
<main class="px-4 py-6 lg:px-6 min-h-[calc(100vh-140px)]">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-[24px] font-semibold text-ink-900">Best Sellers</h1>
            <p class="mt-1 text-[14px] text-ink-500">Manage products featured in the Best Sellers section on the homepage (Max 6 products).</p>
        </div>
        <div>
            <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[13px] font-semibold {{ count($bestSellers) >= 6 ? 'bg-amber-100 text-amber-800' : 'bg-brand-50 text-brand-600' }}">
                <i data-lucide="sparkles" class="h-4 w-4"></i>
                {{ count($bestSellers) }} / 6 Products Added
            </span>
        </div>
    </div>
    ...
    <div class="mb-6 rounded-card border border-surface-line bg-surface-card p-6 shadow-card">
        <h2 class="text-[16px] font-semibold text-ink-900 mb-4">Add Product to Best Sellers</h2>
        ...
        @if (count($bestSellers) >= 6)
            <div class="rounded-base border border-amber-200 bg-amber-50 p-4 text-[14px] text-amber-800 flex items-center gap-3">
                ...
                <span>You have reached the maximum limit of <strong>6 products</strong> for Best Sellers. Remove an existing product to add a new one.</span>
            </div>
        @else
            <form action="{{ route('admin.featured.best-sellers.add') }}" method="POST" class="flex flex-wrap items-end gap-4">
                @csrf
    ...
    <div class="rounded-card border border-surface-line bg-surface-card p-6 shadow-card">
        <h2 class="text-[16px] font-semibold text-ink-900 mb-4">Current Best Seller Products</h2>
                    ...
                    @forelse ($bestSellers as $product)
                        <tr class="hover:bg-surface-body/70 transition-colors">
                            ...
                            <td class="py-4 pr-4 text-right">
                                <form action="{{ route('admin.featured.best-sellers.remove', $product->id) }}" method="POST" onsubmit="return confirm('Remove this product from Best Sellers?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex h-9 px-3 items-center justify-center gap-1.5 rounded-base border border-danger-200 bg-danger-50 text-danger-600 hover:bg-danger-600 hover:text-white transition-colors text-[13px] font-medium" title="Remove from Best Sellers">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-ink-400">
                                ...
                                No Best Seller products added yet. Use the form above to add products.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
