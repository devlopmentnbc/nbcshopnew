...
@section('title', 'Add New Attribute - Admin Unimart')
...
<main class="px-4 py-6 lg:px-6 min-h-[calc(100vh-140px)]">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-[24px] font-semibold text-ink-900">Add New Attribute</h1>
            <p class="mt-1 text-[14px] text-ink-500">Create an attribute (e.g. Size, Weight) and define its values with metrics (e.g. 50 ml, 50 kg).</p>
        </div>
        <div>
        ...
        <form action="{{ route('admin.attributes.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="e.g. Size, Weight, Volume" class="h-11 w-full rounded-base border border-surface-line bg-surface-body px-4 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none @error('name') border-danger-500 @enderror">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="e.g. size, weight" class="h-11 w-full rounded-base border border-surface-line bg-surface-body px-4 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none @error('slug') border-danger-500 @enderror">
                    @error('slug')
                        <p class="mt-1.5 text-[13px] text-danger-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-[13px] text-ink-500">Define the values for this attribute (e.g. Value Name: "50 ml", Value: "50", Metric: "ml").</p>
                    </div>
                    ...
                    <div class="value-row flex flex-wrap items-center gap-3 rounded-base border border-surface-line bg-surface-body p-3">
                        ...
                        <div class="w-40 min-w-[120px]">
                            ...
                            <select name="values[0][metric]" class="h-10 w-full rounded-base border border-surface-line bg-surface-card px-3 text-[14px] text-ink-900 focus:border-brand-600 focus:outline-none">
                                ...
                                <option value="ml" selected>ml (Milliliter)</option>
